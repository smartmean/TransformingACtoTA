...
class UppaalConverter:
    ...
    def add_location(self, template, node_id, node_name, node_type):
        ...
        loc_id = node_id #กำหนดให้ loc_id เท่ากับ node_id
        template['state_map'][node_id] = loc_id #เพิ่มข้อมูลเข้าไปใน state_map ของ template ที่มีค่าเป็นตัวแปรที่ระบุตำแหน่งของ node ใน Activity Diagram

        x = template['x_offset'] #กำหนดให้ x เท่ากับ x_offset ของ template
    ...
    def create_fork_template(self, template_name, fork_id, outgoing_edge, node_info, node_types):
        ...
        while current_node and current_node not in local_processed_nodes:
            ...
            current_node_name = node_info.get(current_node, "")
    ...
    def add_transition(self, template, source_id, target_id, source_name="", target_name="", target_type=""):
        ...
        if source and target:
            ...
            if source_type in ("uml:ForkNode", "ForkNode"):
                ...
                if self.node_types.get(edge_target) == "uml:JoinNode":
                    print(f"Target ID {edge_target} is a JoinNode")
                    guard_conditions = []
                    #print(f"Checking guard conditions for target_id: {target_id}, type: {self.node_types.get(target_id)}")
                    #print(f"Outgoing edges: {outgoing_edges}")

                    for i in range(len(outgoing_edges)):
                        template_name = f"Template{i+1}"
                        if f"bool Done_{template_name};" not in self.declarations:
                            print(f"Error: Done_{template_name} is not declared before use")
                        guard_conditions.append(f"Done_{template_name}==true")
                        print(f"guard_conditions: {guard_conditions};")
