...
class UppaalConverter:
    ...
    def create_fork_template(self, template_name, fork_id, outgoing_edge, parent_template=None, level=0):
        ...
        print(f"DEBUG: Template {hierarchical_name} branch nodes: {[self.parser.get_node_name(nid) for nid in branch_nodes]}")
    ...
    def _create_template_transitions_clean(self, fork_template, initial_id, template_name, level, branch_nodes):
        ...
        # สร้าง fork synchronization transition จาก nested ForkNode ไป corresponding JoinNode
        if nested_fork_id and corresponding_join_id and level == 0:  # เฉพาะ top-level templates
            ...
            # สร้าง transition จาก nested ForkNode ไป corresponding JoinNode
            transition = ET.SubElement(fork_template["element"], "transition")
            ET.SubElement(transition, "source", ref=nested_fork_id)
            ET.SubElement(transition, "target", ref=corresponding_join_id)
            
            x1, y1 = fork_template["position_map"].get(nested_fork_id, (0, 0))
            x2, y2 = fork_template["position_map"].get(corresponding_join_id, (0, 0))
            x_mid = (x1 + x2) // 2
            y_mid = (y1 + y2) // 2
            
            # เพิ่ม synchronization label
            ET.SubElement(transition, "label", kind="synchronisation", x=str(x_mid), y=str(y_mid - 80)).text = f"{nested_channel}!"
            ...
            for i in range(len(outgoing_edges)):
                nested_template_name = f"{template_name}_NestedTemplate{level+1}_{i+1}"
                guard_conditions.append(f"Done_{nested_template_name}==true")
            
            if guard_conditions:
                ET.SubElement(transition, "label", kind="guard", x=str(x_mid), y=str(y_mid - 60)).text = " && ".join(guard_conditions)
