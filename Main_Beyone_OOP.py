"""
FastAPI Application ที่ใช้ OOP-designed UPPAAL Converter
รักษา API interface เดิมไว้ แต่ใช้โครงสร้างใหม่ที่ปรับปรุงแล้ว
"""
...
class UppaalConverter:
    """ แปลง Activity Diagram XML → UPPAAL XML (Compatible with original) """
    ...
    def convert(self, xml_content: str) -> str:
        """แปลง XML content เป็น UPPAAL XML"""
        # Parse XML
        self.activity_root = ET.fromstring(xml_content)
        ...
        if source and target:
            ...
            if (template["name"] == "Template" and 
                ...
                for edge in self.activity_root.findall(".//{*}edge"):
                    ...
                    if (edge_source == target_id and 
                        self.node_types.get(edge_target) == "uml:JoinNode"):
...
async def convert_xml(file: UploadFile = File(...)):
    """
    API Endpoint สำหรับการแปลง Activity Diagram XML เป็น UPPAAL XML
    ใช้ OOP-designed converter
    """
    try:
        # อ่านไฟล์ XML
        ...
        # แปลง XML
        uppaal_xml = converter.convert(xml_content)
